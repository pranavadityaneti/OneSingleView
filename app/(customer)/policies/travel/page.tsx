...
import { getUserTravelPolicies } from '@/lib/db';
import { User } from '@/types';
import { calculatePolicyStatus, formatCurrency } from '@/lib/utils';
import { Plane, Plus, ArrowLeft, Search, Loader2 } from 'lucide-react';
...
export default function TravelPoliciesPage() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const travelPolicies = await getUserTravelPolicies(currentUser.id);
                const processedPolicies = travelPolicies.map((p: any) => ({
                    ...p,
                    status: calculatePolicyStatus(new Date(p.policy_end_date))
                }));
                setPolicies(processedPolicies);
            } catch (error) {
                console.error('Error loading travel policies:', error);
            } finally {
                setLoading(false);
            }
        };
    ...
    const filteredPolicies = policies.filter(policy =>
        ...
        policy.destination?.toLowerCase().includes(searchQuery.toLowerCase())
    );
        ...
        <div className="space-y-6">
            ...
            {filteredPolicies.length === 0 ? (
                <div className="text-center py-12 bg-white rounded-xl border border-gray-200 border-dashed">
                    ...
                    <h3 className="text-lg font-medium text-gray-900">No travel policies found</h3>
                    <p className="text-gray-500 mt-1">
                        {searchQuery ? 'Try adjusting your search' : 'Add your first travel policy to get started'}
                    </p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredPolicies.map((policy) => {
                        ...
                        return (
                            <div
                                key={policy.id}
                                className="bg-white rounded-xl border border-gray-200 hover:border-cyan-300 hover:shadow-md transition-all p-5"
                            >
                                ...
                                <p className="text-xs font-medium text-gray-600 mb-3">Travel Insurance</p>

                                <div className="space-y-2 text-sm">
                                    <div className="flex justify-between">
                                        <span className="text-gray-500">Destination</span>
                                        <span className="font-medium">{policy.destination || 'N/A'}</span>
                                    </div>
                                    <div className="flex justify-between">
