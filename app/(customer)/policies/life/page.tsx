...
import { getUserLifePolicies } from '@/lib/db';
import { User } from '@/types';
import { calculatePolicyStatus, formatCurrency } from '@/lib/utils';
import { Umbrella, Plus, ArrowLeft, Search, Loader2 } from 'lucide-react';
import Link from 'next/link';

export default function LifePoliciesPage() {
    ...
    useEffect(() => {
        const loadData = async () => {
            try {
                ...
                const lifePolicies = await getUserLifePolicies(currentUser.id);
                const processedPolicies = lifePolicies.map((p: any) => ({
                    ...p,
    ...
    const filteredPolicies = policies.filter(policy =>
        ...
        policy.nominee_name?.toLowerCase().includes(searchQuery.toLowerCase())
    );
                ...
                <div className="flex items-center gap-4">
                    ...
                    <div>
                        <h1 className="text-2xl font-bold text-gray-900 flex items-center gap-2">
                            <Umbrella className="w-7 h-7 text-pink-600" />
                            Life Policies
                        </h1>
            ...
            {filteredPolicies.length === 0 ? (
                <div className="text-center py-12 bg-white rounded-xl border border-gray-200 border-dashed">
                    <div className="w-12 h-12 bg-pink-50 rounded-full flex items-center justify-center mx-auto mb-3">
                        <Umbrella className="w-6 h-6 text-pink-600" />
                    </div>
                    <h3 className="text-lg font-medium text-gray-900">No life policies found</h3>
                    <p className="text-gray-500 mt-1">
                        {searchQuery ? 'Try adjusting your search' : 'Add your first life policy to get started'}
                    </p>
                </div>
            ) : (
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                    {filteredPolicies.map((policy) => {
                        ...
                        return (
                            <div
                                key={policy.id}
                                className="bg-white rounded-xl border border-gray-200 hover:border-pink-300 hover:shadow-md transition-all p-5"
                            >
                                ...
                                <p className="text-xs font-medium text-gray-600 mb-3">Life Insurance</p>

                                <div className="space-y-2 text-sm">
                                    <div className="flex justify-between">
                                        <span className="text-gray-500">Nominee</span>
                                        <span className="font-medium">{policy.nominee_name || 'N/A'}</span>
                                    </div>
                                    <div className="flex justify-between">
